...
namespace lslidar_driver
{
	...
	bool LslidarDriver::initialize()
	{
		...
		RCLCPP_INFO(this->get_logger(), "Initialised lslidar is Successfully ");
		return true;
	}
	...
	void LslidarDriver::pubScanThread()
	{
		...
		while (rclcpp::ok())
		{
			...
			{ 
				...
				{
					auto scan = sensor_msgs::msg::LaserScan::UniquePtr(new sensor_msgs::msg::LaserScan());
					// Keep LaserScan array length consistent with the angle metadata.
					// For a full 360-degree scan, the first and last bin represent the same ray.
					int scan_num = ceil((angle_able_max - angle_able_min) / 360 * count_num);
