...
std::string g_usart_port_name = "/dev/ttyACM0";
int g_serial_baud_rate = 115200;
...
void origincar_base::Publish_ImuSensor()
{
    ...
    Imu_Data_Pub.orientation_covariance[8] = 0.03;
    ...
    Imu_Data_Pub.angular_velocity_covariance[8] = 0.02;
...
void origincar_base::Publish_Odom()
{
    ...
    memcpy(odom.pose.covariance.data(), odom_pose_covariance, sizeof(odom_pose_covariance));


    odom.twist.twist.linear.x =  Robot_Vel.X;
    odom.twist.twist.linear.y =  Robot_Vel.Y;
    odom.twist.twist.angular.z = Robot_Vel.Z; 
    memcpy(odom.twist.covariance.data(), odom_twist_covariance, sizeof(odom_twist_covariance));

    odom_tf.header.stamp = odom.header.stamp;
    odom_tf.header.frame_id = odom_frame_id;
    odom_tf.child_frame_id = robot_frame_id;
...
: rclcpp::Node ("origincar_base")
{
  ...
  this->get_parameter("publish_odom_tf", publish_odom_tf);
  g_usart_port_name = usart_port_name;
  g_serial_baud_rate = serial_baud_rate;
...
void sigintHandler(int sig)
{
    ...
    Stm32_Serial.setPort(g_usart_port_name);
    Stm32_Serial.setBaudrate(g_serial_baud_rate);
    serial::Timeout _time = serial::Timeout::simpleTimeout(2000);
    Stm32_Serial.setTimeout(_time);
    Stm32_Serial.open();                                       
    SEND_DATA Send_Data;
